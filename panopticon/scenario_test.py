...
def run_scenario():
    ...
    if resp.status_code == 200:
        ...
        print(f"    Graph Context: {len(data['graph'].get('nodes', {}))} nodes, {len(data['graph'].get('edges', []))} edges.")
        
        # Check for the link
        nodes = data['graph'].get('nodes', {})
        if any("gamer_elite@yahoo.com" in str(props) for props in nodes.values()):
            print("    [SUCCESS] Pivot Successful! Linked 'cipher_v' to 'gamer_elite' via Password/IP.")
        else:
