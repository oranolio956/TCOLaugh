...
def get_stats():
    ...
    docs = cur.execute("SELECT COUNT(*) FROM documents WHERE source_type != 'audit_log'").fetchone()[0]
    audits = cur.execute("SELECT COUNT(*) FROM documents WHERE source_type = 'audit_log'").fetchone()[0]
    ...
    if graph_context:
        for uid, info in graph_context.get('nodes', {}).items():
            ...
            if info['type'] == 'IPAddress':
                ...
                locations.append({"ip": ip, "lat": lat, "lon": lon, "country": country})
...
async def search_face(file: UploadFile = File(...)):
    try:
        ...
        for det in detections:
            ...
            import numpy as np
            vec_matches = db_instance.search_vectors(np.array(emb, dtype=np.float32))
            matches.append({"face_score": det['detection_score'], "db_matches": vec_matches})
...
def active_recon(request: ReconRequest):
    ...
    db_instance.add_document(doc_id, "active_recon", 0, {"username": request.username, "hits": hits})
