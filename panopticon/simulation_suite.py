...
class UserAgent:
    ...
    def _do_recon(self, username: str):
        resp = requests.post(
            f"{API_URL}/recon/username", json={"username": username}, headers=HEADERS
        )
        if resp.status_code == 200:
...
def inject_seed_data(num_records=100):
    ...
    for i in range(1, 51):  # Match the 50 agents
        ...
        db_instance.add_node(
            f"user:target_user_{i}", "Identity", {"username": f"target_user_{i}"}
        )
        ...
        db_instance.add_node(
            f"email:target_{i}@shadow.net", "Email", {"val": f"target_{i}@shadow.net"}
        )
