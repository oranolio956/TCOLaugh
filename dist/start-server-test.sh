...
sed -i "s/\"port\": [0-9][0-9]*/\"port\": $PORT/g" ./profile.json

# Verify the port was updated
echo "Profile port after update:"
grep "port" ./profile.json
...
sed -i "s/\"port\": 10000/\"port\": $PORT/g" ./profile_render.json

# Start the server using the new profile
echo "Starting server with HTTP configuration..."
exec ./adaptixserver -profile ./profile_render.json